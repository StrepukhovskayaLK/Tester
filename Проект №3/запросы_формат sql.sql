...
/*Составьте запрос, который выведет имя вида, начинающегося на «п» и относящегося к типу 
...
SELECT species_name, species_amount , to_char(date_start,'MM'), place_name
FROM species 
INNER JOIN species_in_places
ON species.species_id=species_in_places.species_id
...
select s.*,
t.species_name AS parent_name
from species AS s
left join species AS t
ON s.date_start=t.date_start
where t.species_name='малыш';




2.	/*Составьте запрос, который выведет название вида, расположенного в здании с наибольшей 
площадью. Результат будет соответствовать букве «Ж»*/
...
3.	/*Составьте запрос/запросы, которые найдут название вида, относящегося к 5-й по 
численности группе проживающей дома. Результат будет соответствовать букве «Ш»*/
Подсказка:
Сначала сгруппируйте виды по типу и найдите id типа. Затем среди всех представителей типа
найдите тот вид, который проживает дома.


SELECT species_name, species_amount , place_name
FROM species 
INNER JOIN species_in_places
ON species.species_id=species_in_places.species_id
...
4. /*Составьте запрос, который выведет сказочный вид (статус fairy), не расположенный 
ни в одном месте. Результат будет соответствовать букве «Т»*/

SELECT species_name, species_status, place_name
FROM species 
